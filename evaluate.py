...
from data import Train_Loader, Test_Loader, post_RS_normalisation
from utils import adjust_lr, AvgMeter, compute_bce_individual, compute_entropy_individual
    ...
    for epoch in range(1, (param.epoch + 1)):
        ...
        with tqdm(train_loader, unit='batch') as pbar:
            for pack in pbar:
                ...
                for k in range(param.M):
                    images_k = imgs_copy.clone().detach()
                    noise = torch.rand_like(images_k)

                    # means = torch.zeros_like(images_k)
                    # stds = torch.ones_like(images_k)
                    ...
                    # smoothing_factor = RF.radiate((scale / param.eta)**2)
                    ...
                    # 2nd Version
                    ###########################################################################
                    # radiation_factor = RF.radiate(phi)
                    # inversed_smoothing_scale = param.smooth_radiation * (param.noise_sd - phi)**2
                    # inversed_smoothing_scale = param.smooth_radiation * ((param.noise_sd - phi) / param.noise_sd)**2

                    # smoothing_factor = radiation_factor + inversed_smoothing_scale
                    ###########################################################################
                    # Original Code
                    ###########################################################################
                    ...
                    # smoothing_factor = radiation_factor

                    # gts_k = label_smoothing(gts_k, smoothing_factor)
                    # gts = torch.cat((gts, gts_k), dim = 0)                    

                # images = post_RS_normalisation(images)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_group', type=str, default='Comparison_Models', help='experiment group name')
    parser.add_argument('--exp_name', type=str, default='EBMGSOD', help='experiment name')
    ...
    if pm.test_dataset == 'All':
        test_datasets = ['DUTS-TE', 'DUT-OMRON', 'PASCAL-S', 'SOD', 'ECSSD', 'HKU-IS']
    else:
        test_datasets = [pm.test_dataset]


    # unnormalised = pd.DataFrame(columns=['Dataset', 'ACC', 'ECE', 'OE'])
    # normalised = pd.DataFrame(columns=['Dataset', 'ACC', 'ECE', 'OE'])
    ...
    #                           method=pm.exp_name, 
    #                           n_bins=10)
        # df_row = {'Dataset' : dataset, 'ACC' : acc, 'ECE' : ece, 'OE': oe}
        # unnormalised = unnormalised.append(df_row, ignore_index=True)
        # ECE_EM(dir_pred=dir_pred, dir_gt=dir_gt,param=pm, dataset=dataset, method=pm.exp_name, n_bins=10)
        # # ECE_SWEEP(dir_pred=dir_pred, dir_gt=dir_gt,param=pm, dataset=dataset, method=pm.exp_name, n_bins=100)
        # ECE_DEBIAS(dir_pred=dir_pred, dir_gt=dir_gt,param=pm, dataset=dataset, method=pm.exp_name, n_bins=10)
    ...
    #                           norm=True)
    #     df_row = {'Dataset' : dataset, 'ACC' : acc, 'ECE' : ece, 'OE': oe}
    #     normalised = normalised.append(df_row, ignore_index=True)
    #     # ECE_EM(dir_pred=dir_pred, dir_gt=dir_gt,param=pm, dataset=dataset, method=pm.exp_name, n_bins=10)
    #     # # ECE_SWEEP(dir_pred=dir_pred, dir_gt=dir_gt,param=pm, dataset=dataset, method=pm.exp_name, n_bins=100)
    #     # ECE_DEBIAS(dir_pred=dir_pred, dir_gt=dir_gt,param=pm, dataset=dataset, method=pm.exp_name, n_bins=10)

    # unnormalised.to_excel("./experiments/{}/{}/Calibration_Unnormalised_{}.xlsx".format(pm.exp_group, pm.exp_name, pm.exp_name), 
    #                       sheet_name="Unnormalised")
    # unnormalised.to_excel("./experiments/{}/{}/Calibration_Normalised_{}.xlsx".format(pm.exp_group, pm.exp_name, pm.exp_name), 
